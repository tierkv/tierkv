...
//!
//! Both services are defined in proto/hailey.proto and compiled by build.rs.
...
impl ColdVaultClient {
    ...
    /// Evict a KV tensor to cold storage on Mac Air.
...
impl RecomputeClient {
    ...
    /// Ask DGX Spark to recompute a KV tensor from weights (cache miss path).
...
// ── Cold Vault server (runs on Mac Air) ───────────────────────────────────────
...
// ── Recompute server stub (runs on DGX Spark, plugged into EXO later) ────────
