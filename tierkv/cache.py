"""
Thin Python wrapper around the Rust tierkv_core extension.

Usage:
    from tierkv import get_cache

    cache = get_cache(dim=256)
    cache.insert(token_idx=0, layer=3, data=kv_bytes)
    kv_bytes = cache.get(token_idx=0, layer=3)
    print(cache.stats())
"""

from tierkv_core import TieredKVCache, TurboQuant

_COLD_NODE      = "127.0.0.1:50051"
_RECOMPUTE_NODE = "127.0.0.1:50052"
...
def get_cache(
    ...
    Args:
        dim:            KV vector dimension (must be divisible by 8).
                        Typical values: 256 (Qwen3.6-35B-A3B head_dim),
                        2048 (Qwen 7B), 4096 (Qwen 72B).
        hot_capacity:   Max token×layer slots held in hot tier.
        cold_node:      gRPC address for cold vault (host:port).
        recompute_node: gRPC address for KV recompute (host:port).

    Returns:
        TieredKVCache ready to call .insert() / .get() / .get_batch() / .stats()
    """
